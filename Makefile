...
build: clean
	mkdir -p tmp/$(NAME)
	rsync -r --prune-empty-dirs --exclude=".*" --exclude=tmp --exclude=images --exclude=misc --exclude=Makefile --exclude=description.bbc --exclude=VERSION --exclude=ck3-tiger.conf . tmp/$(NAME)

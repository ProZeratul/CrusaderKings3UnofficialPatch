---
name: unop-review-fix
description: Adversarially review the most recent fix for a confirmed Unop issue
argument-hint: "<issue-number>"
---

# Review Unop Issue Fix

Adversarially review the fix for a confirmed Unop issue. Read the full context from a shared file, review both the fix approach and the diff against `main`, and rate each finding by severity. Append a `## Review N` section to the shared file, post a concise verdict comment on the PR, and apply a verdict label to the issue.
...
- **GitHub repo**: `ProZeratul/CrusaderKings3UnofficialPatch`.
- **Issue**: A bug report (or occasionally an enhancement request) opened against the Unop repo.
- **Shared file**: `tmp/issue-<N>.md` — the handover document. Other skills may create it with the `## Analysis` section and append `## Fix N` sections. This skill reads both and appends `## Review N`.
...
- **Issue number** (position $0, required): The GitHub issue number whose fix to review (e.g. `419`).
- **Identity** (position $1): the role this skill posts as, defaults to `reviewer`.
...
- **Read the shared file in full** — the `## Analysis` section and every prior `## Fix N` and `## Review N` — before reviewing.
...
Count the existing `## Fix N` sections — this review is `## Review N` for the same N. Review only the latest fix; earlier `## Fix N` / `## Review N` sections are history for context.
...
Get the diff of the fix branch (the current branch) against `main`, and read the relevant parts of the affected Unop and vanilla files for context beyond the diff:
...
### 6. Post a Comment and Apply the Verdict Label

Post a **concise** `**<identity>:**` comment on the PR (resolve `<PR>` with `gh pr view --json number -q .number` on the checked-out branch): the verdict, then one line per `High` or `Medium` finding.

Apply your verdict label to the **issue** — `review:looks-good` or `review:needs-work`, removing the other if present.

```shell
gh pr comment <PR> --body "$(cat <<'EOF'
**<identity>:** <Looks good | Needs work> — <one-line gist>

- <High/Medium finding, one line each>
EOF
)"
gh issue edit <N> --add-label "review:needs-work" --remove-label "review:looks-good"
```
